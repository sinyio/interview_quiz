...
export const ChooseComplexity = () => {
  ...
  const onChooseComplexity = (id: number) => {
    ...
    const isRangeSelected = range.some((item) =>
      currentComplexity.includes(item)
    );
    ...
    const updates = isRangeSelected
      ...
      : [
          ...currentComplexity,
          ...range.filter((num) => !currentComplexity.includes(num)),
        ];
  ...
  const preparedData = complexityRanges.map(({ id, title, value }) => ({
    ...
    title,
    isActive:
      filter.complexity?.some((selectedItem) => value.includes(selectedItem)) ||
      false,
  }));
